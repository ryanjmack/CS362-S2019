...
void adventurerEffect(struct gameState* state, int* temphand)
{
...
void smithyEffect(int handPos, struct gameState *state)
{
  int currentPlayer = whoseTurn(state);
...
void councilroomEffect(struct gameState* state, int handPos)
{
  int currentPlayer = whoseTurn(state);
...
void feastEffect(struct gameState* state, int* temphand, int choice1)
{
  int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
      case adventurer:
        adventurerEffect(state, temphand);
        return 0;

    case council_room:
      councilroomEffect(state, handPos);
      return 0;

    case feast:
      feastEffect(state, temphand, choice1);
      return 0;

    case gardens:
      // probable BUG, why is there no behavior for gardens?
      return -1;
